;(function(window){
			...
			OnUCUserReply : BX.delegate(function(entityId, authorId, authorName) {
				if (!!this.entitiesId[entityId]) {
					...
					if (authorId > 0)
					{
						...
						BXfpdSelectCallbackMent(item, 'users', '', this.form.id, this.editorName, true);
					}
				} }, this),
	...
	window.FCForm.prototype = {
		...
		hide : function(quick) { BX.onCustomEvent(this.eventNode, 'OnShowLHE', [(quick === true ? false : 'hide')]); },
		...
		clear : function() {
			...
			var
				...
				node = (!!filesForm ? BX.findChild(filesForm, {'className': 'file-placeholder' }, true, false) : null),
		...
		submit : function(mpfObj)
		{
			...
			var post_data = {};
			convertFormToArray(this.form, post_data, text);
