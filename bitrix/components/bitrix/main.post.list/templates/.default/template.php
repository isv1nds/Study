	...
	<div id="record-#FULL_ID#" class="feed-com-block-outer">
		...
		<div class="feed-com-block feed-com-block-#NEW# blog-comment-user-#AUTHOR_ID# sonet-log-comment-createdby-#AUTHOR_ID# feed-com-block-#APPROVED#">
			...
			<div class="feed-com-informers">
				...
				#BEFORE_ACTIONS#
				<?if ( $arParams["SHOW_POST_FORM"] == "Y" ) { ?><a href="javascript:void(0);" class="feed-com-reply feed-com-reply-#SHOW_POST_FORM#" <?
				?>id="record-#FULL_ID#-actions-reply" <?
				?>onclick="window['UC']['#ENTITY_XML_ID#'].reply(this)" bx-mpl-author-id="#AUTHOR_ID#" bx-mpl-author-name="#AUTHOR_NAME#"><?=GetMessage("BLOG_C_REPLY")?></a><? } ?>
				<a href="#" <?
			...
			<div class="feed-com-text">
				...
				<div class="feed-post-text-more" onclick="fcExpandComment('#FULL_ID#', this)">
...
if (!!$arParams["NAV_STRING"] && !!$arParams["NAV_RESULT"]) {
	...
	if ($count > 0)
	{
		...
		if ($arParams["PREORDER"] == "Y") {?><div id="<?=$arParams["ENTITY_XML_ID"]?>_hidden_records" class="feed-hidden-post" style="display:none; overflow:hidden;"></div> <?}
...
if ($arParams["PREORDER"] != "Y") { ?><?=$arParams["NAV_STRING"]?><? }
...
<?if ($GLOBALS["USER"]->IsAuthorized() && CModule::IncludeModule("pull") && CPullOptions::GetNginxStatus()) { ?>
	BX.addCustomEvent("onPullEvent", function(module_id, command, params) {
		if (module_id == "unicomments" && params["ENTITY_XML_ID"] == '<?=$arParams["ENTITY_XML_ID"]?>') {
...
if ($arParams["SHOW_POST_FORM"] == "Y") {
	?>
		...
		<div class="feed-com-footer" id="record-<?=$arParams["ENTITY_XML_ID"]?>-switcher" onclick="window['UC']['<?=$arParams["ENTITY_XML_ID"]?>'].reply();" <?
			?><?if ($arParams['SHOW_MINIMIZED'] != "Y") { ?> style="display:none;" <? }?>><?
