...
<?endforeach?>
    ...
    <input placeholder="<?=GetMessage("AUTH_LOGIN")?>" type="text" name="USER_LOGIN" maxlength="50" value="<?=$arResult["USER_LOGIN"]?>" size="17" />
    <input placeholder="<?=GetMessage("AUTH_PASSWORD")?>" type="password" name="USER_PASSWORD" maxlength="50" size="17" />

    <noindex><a href="<?=$arResult["AUTH_FORGOT_PASSWORD_URL"]?>" rel="nofollow" class="hd_forgotpassword"><?=GetMessage("AUTH_FORGOT_PASSWORD_2")?></a>
        </noindex>
...
<?if($arResult["NEW_USER_REGISTRATION"] == "Y"):?><noindex>
    <a href="<?=$arResult["AUTH_REGISTER_URL"]?>" rel="nofollow" class="hd_signup"><?=GetMessage("AUTH_REGISTER")?></a></noindex>
    <?endif?>
