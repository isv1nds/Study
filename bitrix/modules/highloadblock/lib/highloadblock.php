...
class HighloadBlockTable extends Entity\DataManager
{
	...
	public static function add($data)
	{
		...
		elseif ($dbtype == 'oracle')
		{
			...
			$GLOBALS['DB']->query('
				CREATE SEQUENCE sq_'.$data['TABLE_NAME'].' INCREMENT BY 1 NOMAXVALUE NOCYCLE NOCACHE NOORDER
			');
	...
	public static function update($primary, $data)
	{
		...
		if ($data['TABLE_NAME'] !== $oldData['TABLE_NAME'])
		{
			...
			elseif ($connection instanceof OracleConnection)
			{
				...
				$connection->query('ALTER TRIGGER '.$oldData['TABLE_NAME'].'_insert RENAME TO '.$data['TABLE_NAME'].'_insert');
			}
		}
