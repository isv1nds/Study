...
window.LHEPostForm.prototype = {
	Init : function(formID, params)
	{
		...
		if (!!params["BFileDLoadFormController"]) {
			...
			if (!!params["FControllerID"]) {
				...
				func = function(obj){if (obj.dialogName == 'AttachmentsDialog') this.FController = obj;};
			...
			BX.addCustomEvent('BFileDSelectFileDialogLoaded', BX.delegate(func, this));
			BX.addCustomEvent(
	...
	OnFileUploadSuccess : function(result, obj)
	{
		...
		if (!!obj && !!obj.CID)
		{
			...
			{
				var node = BX.findChild(BX(this.formID), {'attr': {id: 'upload-cid'}}, true, false);
				if (!!node) { node.value = obj.CID; }
			}
			else if (obj.dialogName == 'AttachFileDialog')
			{
				var node = BX.findChild(BX(this.formID), {'attr': {id: 'upload-wd-cid'}}, true, false);
				if (!!node) { node.value = obj.CID; }
			}
		}
...
window.CustomizeLightEditor = function(editorId, params)
{
	...
	MPFSmileMenu.prototype.mpfOpenSmileMenu = function()
	{
	    if (!BX.proxy_context)
	        return false;
...
// remove block
		...
		if (e.keyCode == 107 || (e.shiftKey || e.modifiers > 3) && BX.util.in_array(e.keyCode, [187, 50, 107, 43, 61]))
		{
			...
			setTimeout(function() {
				...
				if (determiner == "@" || determiner == "+")
				{
					...
					if (!prevS || BX.util.in_array(prevS, ["+", "@", ",", "("]) || (prevS.length == 1 && BX.util.trim(prevS) == 0))
					{
				...
				if(pEditor)
				{
					...
					else if(pEditor.sEditorMode == 'html') // WYSIWYG
					{
						...
						var
							...
							txtPos = (txtPos < 0 ? r.endContainer.length : txtPos),
	...
	window.MPFMentionInit = function(formId, params)
	{
		if (!params["items"]["departmentRelation"])
			params["items"]["departmentRelation"] = buildDepartmentRelation(params["items"]["department"]);
		window["departmentRelation"] = params["items"]["departmentRelation"]; // for calendar - do not remove

		if (params["initDestination"] == true)
		{
			BXSocNetLogDestinationFormName = 'destination' + ('' + new Date().getTime()).substr(6);
			...
			if (!!params["itemsHidden"])
				for (var ii in params["itemsHidden"])
					BXfpdSelectCallback({id:('SG'+params["itemsHidden"][ii]["ID"]), name:params["itemsHidden"][ii]["NAME"]}, 'sonetgroups', '', true);
			BXfpdSetLinkName(BXSocNetLogDestinationFormName);
		}
		window["BXfpdSelectCallbackMent" + formId] = function(item, type, search) {
			BXfpdSelectCallbackMent(item, type, search, formId, params["jsObjName"]);
		}
