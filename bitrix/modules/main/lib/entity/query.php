...
*/
	 ...
	 * @return string
	 */
	public function getInitAlias()
	{
		if ($this->init_alias === null)
		{
			$this->init_alias = strtolower($this->init_entity->getCode()).$this->table_alias_postfix;

			$connection = \Bitrix\Main\Application::getConnection();
			$aliasLength = $connection->getSqlHelper()->getAliasLength();

			if (strlen($this->init_alias) > $aliasLength)
			{
				$this->init_alias = 'base';
			}
		}
