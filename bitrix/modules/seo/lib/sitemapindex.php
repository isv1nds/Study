	...
	extends SitemapFile
{
	...
	public function createIndex($arIndex)
	{
		...
		foreach ($arIndex as $file)
		{
			if(!$file->isSystem() && $file->isExists())
			{
				$str .= sprintf(
					self::ENTRY_TPL,
					Converter::getXmlConverter()->encode($this->settings['PROTOCOL'].'://'.$this->settings['DOMAIN'].$this->getFileUrl($file)),
	...
	public function appendIndexEntry($file)
	{
		if($this->isExists())
		{
			$fileUrlEnc = Converter::getXmlConverter()->encode($this->settings['PROTOCOL'].'://'.$this->settings['DOMAIN'].$this->getFileUrl($file));
