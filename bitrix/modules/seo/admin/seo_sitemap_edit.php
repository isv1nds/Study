...
if($_SERVER['REQUEST_METHOD'] == 'POST' && check_bitrix_sessid() && (strlen($_POST["save"]) > 0 || strlen($_POST['apply']) > 0 || strlen($_POST['save_and_add']) > 0))
{
	$arSitemapSettings = SitemapTable::prepareSettings(array(
		...
		'FILENAME_FILES' => trim($_REQUEST['FILENAME_FILES']),
		'FILENAME_IBLOCK' => trim($_REQUEST['FILENAME_IBLOCK']),
...
?>
	<option value="<?=$hd?>"<?=$domain == $arSitemap['SETTINGS']['DOMAIN'] ? ' selected="selected"' : ''?>><?=$hd?></option>
<?
endforeach;
?>
...
?>
			<tr>
				...
				<td<?=$bActive ? '' : ' style="text-decoration:line-through"'?>>
					...
					<a href="iblock_edit.php?lang=<?=LANGUAGE_ID?>&amp;ID=<?=$d?>&amp;type=<?=$arRes['IBLOCK_TYPE_ID']?>&amp;admin=Y"><?=Converter::getHtmlConverter()->encode($arRes['NAME'].' ('.$arRes['CODE'].')')?></a>
				</td>
