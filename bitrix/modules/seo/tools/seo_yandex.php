...
if(isset($_REQUEST['action']))
{
	...
	if(isset($_REQUEST['domain']) && strlen($_REQUEST['domain']) > 0)
	{
		...
		{
			if($arDomain['DOMAIN'] == $_REQUEST['domain'] && $arDomain['SITE_DIR'] == $_REQUEST['dir'])
			{
				$bFound = true;
				break;
			}
			...
			switch($_REQUEST['action'])
			{
				...
				case 'site_verify':
					...
					if(is_array($arDomain))
					{
						...
						if(isset($arFeeds[$arDomain['DOMAIN']]))
						{
				...
				case 'original_text':
					$textContent = $_REQUEST['original_text'];
...
elseif (isset($_REQUEST['get']))
{
	switch($_REQUEST['get'])
	{
		...
		case 'original_text_form':
			...
			{
				$msg = new CAdminMessage(Loc::getMessage('SEO_YANDEX_ERROR'));
				echo $msg->Show();
			}
			else
			{
