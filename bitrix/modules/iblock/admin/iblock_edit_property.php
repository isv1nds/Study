...
if(CModule::IncludeModule('highloadblock') && isset($_POST['PROPERTY_DIRECTORY_VALUES']) && is_array($_POST['PROPERTY_DIRECTORY_VALUES']) && isset($_POST["HLB_NEW_TITLE"]))
{
	...
	$highBlockName[0] = strtoupper($highBlockName[0]);
	...
	foreach($arFieldsName as $fieldName => $fieldValue)
	{
		...
		{
			$arUserField["EDIT_FORM_LABEL"] = $arUserField["LIST_COLUMN_LABEL"] = $arUserField["LIST_FILTER_LABEL"] = array('ru' => $_POST['PROPERTY_USER_TYPE_SETTINGS']['LANG'][$fieldName]);
		}
	...
	foreach($_POST['PROPERTY_DIRECTORY_VALUES'] as $dirKey => $arDirValue)
	{
		...
		if($result->isSuccess())
		{
			...
			$entity_data_class::add($arDirValue);
		}
	}
}
