...
while($arIBlockProperty = $dbIBlockProperty->Fetch())
{
	...
	$userType = $arIBlockProperty["USER_TYPE"];
	$isMultiply = ($arIBlockProperty["MULTIPLE"] == 'Y') ? true : false;
	$arAllProperties[] = $arIBlockProperty;

	if(($propertyType === 'L' && $userType === NULL) || $userType === 'directory')
	{
		$arResult[] = $arIBlockProperty;
	}
	elseif($propertyType === 'F' /*&& $isMultiply*/)
	{
...
if(!$bReadOnly && check_bitrix_sessid())
{
	...
	if(strlen($_POST['save']) > 0)
	{
		...
		foreach($arCombinationResult as $arPropertySaveValues)
		{
			...
			$arIBlockElementAdd["NAME"] = \Bitrix\Iblock\Template\Engine::process($sku, $title);
			unset($arIBlockElementAdd['PROPERTY_VALUES']["CML2_LINK"]);
			$idNewElement = $obIBlockElement->Add($arIBlockElementAdd);
...
else
{
	...
	foreach($arResult as $key => $arIBlockProp)
	{
		if($arIBlockProp['PROPERTY_TYPE'] == 'L')
		{
			$dbIBlockPropertyEnum = CIBlockPropertyEnum::GetList(array("SORT" => "ASC"), array("PROPERTY_ID" => $arIBlockProp["ID"]));
			while($arIBlockPropertyEnum = $dbIBlockPropertyEnum->Fetch())
			{
				$arResult[$key]["VALUE"][] = $arIBlockPropertyEnum;
			}
		}
		elseif($arIBlockProp['PROPERTY_TYPE'] == 'S')
		{
			if(CModule::IncludeModule('highloadblock') && isset($arIBlockProp["USER_TYPE_SETTINGS"]["TABLE_NAME"]))
			{
				$hlblock = HL\HighloadBlockTable::getList(array("filter" => array("TABLE_NAME" => $arIBlockProp["USER_TYPE_SETTINGS"]["TABLE_NAME"])))->fetch();
				...
				while($arData = $rsData->fetch())
				{
					...
					$arData["PROPERTY_ID"] = $arIBlockProp["ID"];
					$arData["ID"] = $arData["UF_XML_ID"];
					$arData["SORT"] = $arData["UF_SORT"];
					$arResult[$key]["VALUE"][] = $arData;
				}
			}
		}
	}
