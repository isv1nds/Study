...
class CIBlockCMLImport
{
	...
	function ImportMetaData($xml_root_id, $IBLOCK_TYPE, $IBLOCK_LID, $bUpdateIBlock = true)
	{
		...
		if(!$ar)
		{
			$rs = $this->_xml_file->GetList(
				array(),
				array("PARENT_ID" => $xml_root_id, "NAME" => GetMessage("IBLOCK_XML2_OFFERS_CHANGE")),
				array("ID", "ATTRIBUTES")
			);
			$ar = $rs->Fetch();
			$this->next_step["bOffer"] = true;
		...
		if($ar)
		{
			...
			while($ar = $rs->Fetch())
			{
				...
				elseif($ar["NAME"] == GetMessage("IBLOCK_XML2_INHERITED_TEMPLATES"))
				{
					$arIBlock["IPROPERTY_TEMPLATES"] = array();
					$arTemplates = $this->_xml_file->GetAllChildrenArray($ar["ID"]);
					foreach($arTemplates as $TEMPLATE)
					{
						$id = $TEMPLATE[GetMessage("IBLOCK_XML2_ID")];
						$template = $TEMPLATE[GetMessage("IBLOCK_XML2_VALUE")];
		...
		$rs = $this->_xml_file->GetList(
			array(),
			array("PARENT_ID" => $xml_root_id, "NAME" => GetMessage("IBLOCK_XML2_PRODUCTS_SETS")),
			array("ID", "ATTRIBUTES")
		);
		$ar = $rs->Fetch();
		if ($ar)
			...
			while($arParent = $rsParents->Fetch())
			{
				...
				if(isset($arXMLElement[GetMessage("IBLOCK_XML2_ID")]))
				{
					$rsElement = CIBlockElement::GetList(
						array(),
						array("=XML_ID" => $arXMLElement[GetMessage("IBLOCK_XML2_ID")], "IBLOCK_ID" => $this->next_step["IBLOCK_ID"]),
						false, false,
						array("ID", "IBLOCK_ID")
					);
					if ($arDBElement = $rsElement->Fetch())
					{
						...
						if (isset($arXMLElement[GetMessage("IBLOCK_XML2_PRODUCT_SET")]))
						{
							$arFields = array(
							...
							foreach ($arXMLElement[GetMessage("IBLOCK_XML2_PRODUCT_SET")] as $xmlSet)
							{
								$arFields["ITEMS"][] = array(
									"ITEM_ID" => $this->GetElementByXML_ID($arDBElement["IBLOCK_ID"], $xmlSet[GetMessage("IBLOCK_XML2_VALUE")]),
									"SORT" => intval($xmlSet[GetMessage("IBLOCK_XML2_SORT")]),
								);
							}
	...
	function ImportElement($arXMLElement, &$counter, $bWF, $arParent)
	{
		...
		elseif(isset($arXMLElement[GetMessage("IBLOCK_XML2_NAME")]))
		{
			if($arDBElement)
			{
			...
			if(array_key_exists(GetMessage("IBLOCK_XML2_INHERITED_TEMPLATES"), $arXMLElement))
			{
				$arElement["IPROPERTY_TEMPLATES"] = array();
				foreach($arXMLElement[GetMessage("IBLOCK_XML2_INHERITED_TEMPLATES")] as $TEMPLATE)
				{
					$id = $TEMPLATE[GetMessage("IBLOCK_XML2_ID")];
					$template = $TEMPLATE[GetMessage("IBLOCK_XML2_VALUE")];
					if(strlen($id) > 0 && strlen($template) > 0)
		...
		elseif(array_key_exists(GetMessage("IBLOCK_XML2_PRICES"), $arXMLElement))
		{
			//Collect price information for future use
			$arElement["PRICES"] = array();
			if (is_array($arXMLElement[GetMessage("IBLOCK_XML2_PRICES")]))
		...
		elseif(array_key_exists(GetMessage("IBLOCK_XML2_STORES"), $arXMLElement))
		{
			$arElement["STORE_AMOUNT"] = array();
			$rsStores = $this->_xml_file->GetList(
				...
				array(
					...
					"NAME" => GetMessage("IBLOCK_XML2_STORES"),
				),
				array("ID", "ATTRIBUTES")
			);
		}
		elseif(array_key_exists(GetMessage("IBLOCK_XML2_STORE"), $arXMLElement))
		{
			$arElement["STORE_AMOUNT"] = array();
			$rsStores = $this->_xml_file->GetList(
				array("ID" => "asc"),
				array(
					"><LEFT_MARGIN" => array($arParent["LEFT_MARGIN"], $arParent["RIGHT_MARGIN"]),
					"NAME" => GetMessage("IBLOCK_XML2_STORE"),
				),
				array("ID", "ATTRIBUTES")
			);
		}
		...
		if ($rsStores)
		{
			while ($arStore = $rsStores->Fetch())
			{
				if(strlen($arStore["ATTRIBUTES"]) > 0)
				{
					...
					if(
						is_array($info)
						&& array_key_exists(GetMessage("IBLOCK_XML2_STORE_ID"), $info)
						&& array_key_exists(GetMessage("IBLOCK_XML2_STORE_AMOUNT"), $info)
					)
					{
						$arElement["STORE_AMOUNT"][] = array(
							$info[GetMessage("IBLOCK_XML2_STORE_ID")] => $info[GetMessage("IBLOCK_XML2_STORE_AMOUNT")],
						);
					}
				}
			}
	...
	function ImportElementPrices($arXMLElement, &$counter, $arParent = false)
	{
		...
		elseif(array_key_exists(GetMessage("IBLOCK_XML2_STORES"), $arXMLElement) && $arParent)
		{
			$arElement["STORE_AMOUNT"] = array();
			$rsStores = $this->_xml_file->GetList(
				array("ID" => "asc"),
		...
		elseif(array_key_exists(GetMessage("IBLOCK_XML2_STORE"), $arXMLElement) && $arParent)
		{
			$arElement["STORE_AMOUNT"] = array();
			$rsStores = $this->_xml_file->GetList(
				array("ID" => "asc"),
	...
	function ImportSection($xml_tree_id, $IBLOCK_ID, $parent_section_id)
	{
		...
		while($arS = $rsS->Fetch())
		{
			...
			elseif($arS["NAME"] == GetMessage("IBLOCK_XML2_INHERITED_TEMPLATES"))
			{
